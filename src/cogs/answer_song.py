...
5. ephemeral で「🎉 正解です！」/「❌ 不正解です」を本人にのみ返答する
   (description に回答した曲名と祝福/再挑戦メッセージを併記)
...
# 応答メッセージ
# ==================================================
# 本人 ephemeral 応答のテンプレート
# - 正解: 🎉 タイトル + 「正解: {song}」+ 祝福メッセージ
# - 不正解: ❌ タイトル + 「あなたの回答: {song}」+ 再挑戦メッセージ
_CORRECT_TITLE: str = "🎉 正解です！"
_CORRECT_DESCRIPTION_TEMPLATE: str = "**正解:** {song}\n\nおめでとうございます!"
_INCORRECT_TITLE: str = "❌ 不正解です"
_INCORRECT_DESCRIPTION_TEMPLATE: str = (
    "**あなたの回答:** {song}\n\n残念!もう一度チャレンジしてください。"
)
...
class AnswerSongCog(commands.Cog):
    ...
    async def answer(
        ...
        # title に絵文字付きの結果ラベル、description に回答内容とフォロー文言を載せる。
        if is_correct:
            embed = build_success_embed(
                _CORRECT_DESCRIPTION_TEMPLATE.format(song=song),
                title=_CORRECT_TITLE,
            )
        else:
            embed = build_error_embed(
                _INCORRECT_DESCRIPTION_TEMPLATE.format(song=song),
                title=_INCORRECT_TITLE,
            )
        await interaction.response.send_message(
            embed=embed,
            ephemeral=True,
